...
# IMPORTANT Notes for estimation: good settings
  ...
  # issues: 
   ...
   # save(estimated_parameters, file = "estimated_parameters.Rdata")
```

## Results
```{r resultsCalibration}
...
```{r replicationOfRealizedVariance}
vol_up = 0.25
vol_down = 0.15
...
#   line_plot_multiple(title=title,x=data_x,names_y=names_y, legend = legend, xlab = xlab, ylab = ylab, y1=simulated_gbm[,1], y2=simulated_gbm[,2], y3=simulated_gbm[,3], y4=simulated_gbm[,4])
#   
#   # Cox-Ross-Rubenstein Tree to simulate Stock Price Paths
#     tau = 0.5
#     n = 6 # number steps of the tree
#     
#     u_vol_up = exp(vol_up*sqrt(tau/n)) 
#     d_vol_up =exp(-vol_up*sqrt(tau/n))
#     
#     u_vol_down = exp(vol_down*sqrt(tau/n))
#     d_vol_down =exp(-vol_down*sqrt(tau/n))
#     
#     p_vol1 = (exp(r_f*tau/n)-d_vol_up)/ (u_vol_up-d_vol_up) 
#     p_vol2 = (exp(r_f*tau/n)-d_vol_down)/ (u_vol_down-d_vol_down) 
#     
#     tree_index_up = matrix(rep(NA,len=(n+1)*(2^n)), nrow=n+1)
#     tree_index_up[1,1] = S_0
# 
#     tree_index_down = tree_index_up
#     
#        # vol up
#        for (i in 2:(n+1)){
#           for (j in 1:(2^n)) {
#             if (is.na(tree_index_up[i-1,j]) ==FALSE) { # if value previous step not missing then use for getting 2 new point in this step
#               tree_index_up[i,j*2-1] <- tree_index_up[i-1,j]*u_vol_up # up movement point
#               tree_index_up[i,j*2] <-  tree_index_up[i-1,j]*d_vol_up  # down movement point
#             }
#           }
#         }
#     
...
#        for (i in 2:(n+1)){
#           for (j in 1:(2^n)) {
#             if (is.na(tree_index_down[i-1,j]) ==FALSE) { # if value previous step not missing then use for getting 2 new point in this step
#               tree_index_down[i,j*2-1] <- tree_index_down[i-1,j]*u_vol_down # up movement point
#               tree_index_down[i,j*2] <-  tree_index_down[i-1,j]*d_vol_down  # down movement point
#             }
#           }
#        }
#     
...
#     
#   # take random paths
#     index_cols_up = sample(c(1:2^n), 1)
#     path_vol_up = tree_index_up[,index_cols_up]
#     path_vol_up
#     sd(path_vol_up[2:n] / path_vol_up[1:(n-1)])
#     
#     
#     index_cols_down = sample(c(1:2^n), 1)
#     path_vol_down = tree_index_down[,index_cols_down]
