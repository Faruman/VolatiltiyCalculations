...
maturities_calibration = as.Date(v2x_future_maturities)[2:6]
...
# grid search for parameter optima
  # minimum parameter size = 0 
  # define ONLY VALUES -> grid is created out of them 
  kappa = c(1,10,25,40,60,90)
  theta = c(rep(1,length(kappa)),rep(10,length(kappa)),rep(25,length(kappa)),rep(40,length(kappa)),rep(60, length(kappa)),rep(90, length(kappa)))
  sigma = c(rep(1,length(kappa)^2),rep(10,length(kappa)^2),rep(25,length(kappa)^2),rep(40,length(kappa)^2),rep(60, length(kappa)^2),rep(90, length(kappa)^2))
  gridsearch_start <- as.data.frame(cbind(kappa,theta,sigma))
  colnames(gridsearch_start) <- c("kappa_start", "theta_start", "sigma_start")
  
  # if gridsearch disabled: use these starting parms
  kappa_scalar = 25
  theta_scalar  = 25
  sigma_scalar  = 25
...
# estimate parameters for each combintion of maturity date and current date
  ...
  for (i in 1:length(maturities_calibration)) { # loop for maturity dates
    ...
    parms_one_maturity$Min_of_Loss_Function = NA
    parms_one_maturity$convergence_code = NA
    ...
    for (j in 1:length(current_day_estimation)) {   # loop for estimation days
        ...
        # check if option data available, otherwise skip day. Day without future is ok
        if((nrow(input_call)+nrow(input_put))>0) {
          
          zeta = 0 # no estimation of zeta (vola premium)
          method = "RMSE" # method for loss function
          gridsearch_on = TRUE
        ...
        # Optimization: gridsearch or single set of starting parms
        if(gridsearch_on == TRUE){
          
        # gridsearch result dataset for parameters
          gridsearch_results = as.data.frame(matrix(,nrow=nrow(gridsearch_start),ncol= ncol(gridsearch_start)+2))
          colnames(gridsearch_results)= c("kappa_optim", "theta_optim", "sigma_optim","Min_of_Loss_Function", "convergence_code")
          
          for (k in 1:nrow(gridsearch_results)){
            startParms = c(gridsearch_start[k,1],gridsearch_start[k,2],gridsearch_start[k,3])
              ...
              # save parameters in grid
               gridsearch_results[k,1] = optimOut$p1
               gridsearch_results[k,2] = optimOut$p2
               gridsearch_results[k,3] = optimOut$p3
               gridsearch_results[k,4] = optimOut$value
               gridsearch_results[k,5] = optimOut$convcode
          }
          # select parameters with lowest value of loss function. If same value, choose first parameter set
          min_loss_position = which(gridsearch_results$Min_of_Loss_Function == min(gridsearch_results$Min_of_Loss_Function))
          opt_parms = as.vector(gridsearch_results[min_loss_position[1],])
        } 
          

        # if no gridsearch: use single given starting parameters
            else {
              # define starting parameters
              startParms = c(kappa_scalar, theta_scalar, sigma_scalar)  
              ...
              optimOut <- optimx(startParms, function(startParms) loss_function(startParms,input_list=input_list_loss_function), gr = NULL,
                     ...
                     lower = 0, upper = Inf, # parms >0
                     control = list(), hessian = FALSE)
              opt_parms = c(optimOut$p1,optimOut$p2,optimOut$p3,optimOut$value, optimOut$convcode)
            }

        # save estimated parameters
            parms_one_maturity$kappa[j] = opt_parms[1]
            parms_one_maturity$theta[j] = opt_parms[2]
            parms_one_maturity$sigma[j] = opt_parms[3]
            parms_one_maturity$Min_of_Loss_Function[j] = opt_parms[4]
            parms_one_maturity$convergence_code[j] = opt_parms[5]
  ...
  # transform list elements to tibbles
    for (i in 1:length(estimated_parameters)) {
      estimated_parameters[[i]] = as.data.frame(estimated_parameters[[i]])
    }
...
# IMPORTANT Notes for estimation: good settings
  ...
  # issues: 
   ...
   save(estimated_parameters, file = "./r_output/estimated_parameters_RMSE_FW.Rdata")
```
...
# key data plots
  ...
  parms_plot_date_1 = do.call(cbind.data.frame, parms_plot_date_1) # transform to pure data frame for referencing
  parms_plot_date_2 = do.call(cbind.data.frame, parms_plot_date_2) # transform to pure data frame for referencing
  colnames(parms_plot_date_1) = c("kappa", "theta", "sigma")
  colnames(parms_plot_date_2) = c("kappa", "theta", "sigma")
...
# term structure forwards - model vs observed
  ...
  # get model term structure
    ...
    parms_term_struc_1 <- data.frame(t(matrix(unlist(parms_term_struc_1), nrow=length(parms_term_struc_1), byrow=T))) # transform to pure data frame for referencing
    parms_term_struc_2 <- data.frame(t(matrix(unlist(parms_term_struc_2), nrow=length(parms_term_struc_2), byrow=T))) # transform to pure data frame for referencing
    colnames(parms_term_struc_1) = c("kappa", "theta", "sigma")
    colnames(parms_term_struc_2) = c("kappa", "theta", "sigma") 
...
### MXWO Derivatives
  ...
  # scale prices with ratio VSTOXX VMSCI
